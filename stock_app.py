    ...
    if st.button("🚀 執行全球同步掃描", use_container_width=True):
        ...
        for r in stocks:
            ...
            try:
                inf = tk.info
                # --- 名稱分流邏輯 ---
                # 港股優先取 shortName (通常含中文)，美股取 longName
                if is_hk:
                    nm = inf.get('shortName', inf.get('longName', s_orig))
                else:
                ...
                # --- UI: 統一 H4 字體大小 (與 Expander 文字 100% 一致) ---
                ...
                if rr > 2: adv += " 現價回報比極高，建議積極配置。"
                elif rr > 1: adv += " 走勢穩健，建議長線分批吸納。"
                else: adv += " 接近分析師阻力位，建議觀望。"
                if p_pct < -15: adv = f"🆘 **止蝕警告：** 虧損 {p_pct:.1f}%，請嚴守 **{low:.2f}** 支撐。"
                ...
                res.append({"代號": s_orig, "名稱": nm, "盈虧%": round(p_pct, 1), "R/R": rr})
                st.divider()
            except: 
