...
for _, row in df_data.iterrows():
    ...
    total_val_hkd += (now_p * float(row["數量"])) * rate
    total_cost_hkd += (float(row["成本"]) * float(row["數量"])) * rate
    results.append({"sym": sym, "now": now_p, "cost": float(row["成本"]), "data": data})
...
st.markdown(f"""
    <div style="background-color: #1e1e1e; padding: 20px 15px; border-radius: 12px; text-align: center; margin-bottom: 15px;">
        <div style="color: #ffffff; font-size: 13px; margin-bottom: 5px;">資產總市值 (HKD)</div>
        <div style="color: #ffffff; font-size: 32px; font-weight: 800;">${total_val_hkd:,.0f}</div>
        <div style="color: {'#ff4b4b' if profit_pct < 0 else '#00c853'}; font-size: 18px; font-weight: 700;">{profit_pct:+.2f}%</div>
    </div>
""", unsafe_allow_html=True)
...
for item in results:
    ...
    # 分析結論區 - 採用統一字體與結構 (對齊優化)
    if item["data"]:
        st.markdown(f"""
            <div class="analysis-box">
                <div class="analysis-text">
                    <span>📍 <span class="label">支撐:</span> {unit}{item['data']['support']:.2f}</span>
                    <span>🎯 <span class="label">目標:</span> {unit}{item['data']['target']:.2f}</span>
                </div>
            </div>
        """, unsafe_allow_html=True)
