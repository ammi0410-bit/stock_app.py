    ...
    if st.button("🚀 執行全球同步掃描", use_container_width=True):
        ...
        for _, r in st.session_state.df.iterrows():
            ...
            try:
                ...
                # 名稱與基本計算
                nm = HK_NAMES.get(sym, inf.get('shortName', sym))
                p_pct = (cp - bp) / bp * 100
                ...
                rec = str(inf.get('recommendationKey', 'NONE')).upper()
                
                # --- UI 渲染 (精確對齊截圖排版) ---
                st.markdown(f"#### 💎 {sym} | {nm}")
                st.markdown(f"#### 現價: HK${cp*rate:.2f} | 買入價: HK${bp*rate:.2f}")
                
                color_code = "🟢" if p_pct >= 0 else "🔴"
                st.markdown(f"**[ 評級: {rec} | R/R: {rr} | 盈虧: {color_code} {p_pct:.1f}% ]**")
                
                if p_pct < -15:
                    st.error(f"🚨 止蝕警告：虧損達 {p_pct:.1f}%！目前支撐位在 {low:.2f}，跌破請考慮減倉。")
                else:
