...
with t1:
    if st.button("🚀 啟動全資產掃描", use_container_width=True):
        ...
        # --- 新增：頂部資產佔比看板 ---
        st.subheader("💰 全球資產配置概覽")
        col_a, col_b = st.columns([1, 2])
        with col_a:
            st.metric("總資產價值 (HKD)", f"${total_hkd:,.0f}")
        with col_b:
            pie_df = pd.DataFrame([{"股票": x["nm"], "價值": x["val_hkd"]} for x in results])
            fig_pie = px.pie(pie_df, values='價值', names='股票', hole=.4, height=250)
        ...
        for item in results:
            ...
            # 判斷是否跌穿支撐位 (止蝕預警)
            is_danger = cp <= y_low * 1.02 # 距離支撐位不到 2% 觸發預警
            bg_color = "#ffebee" if is_danger else "#e8f5e9"
            border_color = "#c62828" if is_danger else "#2e7d32"
            ...
            st.markdown(f"""
                ...
                <div style="background-color: {bg_color}; padding: 12px; border-radius: 6px; border-left: 6px solid {border_color}; margin-top: 10px;">
                    <p style="color: {text_color}; {common_style} margin: 0; white-space: nowrap;">
                        {'⚠️ 警告：已接近/跌穿支撐位！' if is_danger else '📍 分析：'} {nm} 支撐 {curr}{y_low:.2f}，目標 {curr}{y_high:.2f}。
                    </p>
                </div>
            """, unsafe_allow_html=True)
            
            with st.expander("📈 走勢詳情"):
                fig = go.Figure(go.Scatter(x=h.index, y=h['Close'], line=dict(color=border_color)))
