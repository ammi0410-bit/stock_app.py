    ...
    if st.button("🚀 執行全球同步掃描", use_container_width=True):
        ...
        for _, r in st.session_state.df.iterrows():
            ...
            try:
                ...
                # --- 分析引擎：港美通用支撐位與目標價計算 ---
                # 優先使用官方數據，若無則使用 52 週高低位模擬
                tgt = inf.get('targetMeanPrice') or inf.get('fiftyTwoWeekHigh') or (cp * 1.15)
                low = inf.get('targetLowPrice') or h['Low'].min() or (cp * 0.85)
                rec = str(inf.get('recommendationKey', 'HOLD')).upper()
                ...
                # HTML 確保現價行與評級行視覺一致
                row_css = "font-size: 16px; font-weight: bold; font-family: sans-serif;"
                st.markdown(f"""
                <div style="{row_css} margin-bottom: 5px;">
                    現價: {curr}{cp:.2f} | 買入價: {curr}{bp:.2f}
                </div>
                <div style="{row_css} color: {'#ff4b4b' if p_pct < 0 else '#00c853'};">
                    [ 評級: {rec} | R/R: {rr} | 盈虧: {'🔴' if p_pct < 0 else '🟢'} {p_pct:.1f}% ]
                </div>
                """, unsafe_allow_html=True)
                
                # 強制顯示分析結果 (包含支撐位)
                st.success(f"📍 分析：{nm} 目前支撐位在 {curr}{low:.2f}，目標價 {curr}{tgt:.2f}。")
